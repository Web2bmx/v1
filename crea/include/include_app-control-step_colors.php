<div>
	<h2>Ahora elige los colores de tu página</h2>
	<div class="container-input"> 
		<label for="inp-color">Selecciona el color principal para el sitio web de tu Negocio</label>
		<input type="color" id="inp-color" name="inp-color" value="#ff0000">
		<p class="app-control-step-tooltip-info">Usaremos este color para generar automáticamente una paleta de colores para el sitio web de tu Negocio.</p>
	</div>
	<div class="container-input">
		<label for="app-control-palettes">Elige la paleta de colores acorde con la personalidad de tu Negocio</label>
		<div id="app-control-palettes"></div>
	</div>	
</div>